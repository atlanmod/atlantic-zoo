module Sigkdd {
	...
	type Person : Thing {
		...
		Add_Ref : Person?;
		Added_by_Ref : Person?;
		Pay_Ref : Registration_fee?;
		Can_stay_in_Ref : Place?;
		Nation_Att : Text;
		Name_Att : Text;
		HasEmail_Att : Text;
		HasName_Att : Text;
	...
	type Conference : Scientific_Event, Thing, Event, Working_event {
		...
		Date_ContRef : Date?;
		Holded_by_Ref : ACM_SIGKDD?;
		Name_of_conference_Att : Text;
		Start_of_conference_Att : Logical;
		City_of_conference_Att : Text;
		End_of_conference_Att : Logical;
	...
	type Paper : Domain_Topic, ActivitySubmissionForm, Document, Contribution, Article, Regular_contribution {
	...
	type Author : ConferenceMember, People, Speaker, Person, User {
		...
		Obtain_Ref : Award?;
		Submit_Ref : Paper?;
		Notification_until_Ref : Deadline_Author_notification?;
	...
	type Document : Thing, Item {
		...
		Presentationed_by_Ref : Speaker?;
		Submit_until_Ref : Deadline?;
	...
	type Abstract : Extended_abstract, Paper, Contribution_File, Document {
	...
	type Presentation : Conference_contribution, Conference_activity {
		Is_given_by_Ref : Active_conference_participant?;
	};

	type Chair : Committee_member, ProgramCommitteeMember {
	...
	type EEnum {"male", "female", "high", "medium", "low", "very high", "1", "2", "3", "4", "5", "6", "7", "submitted", "missing", "notStarted", "active", "finished", "accepted", "rejected", "regular", "invited", "lowBid", "-", "CONFLICT", "wantNot", "highBid", "Yes", "No", "Unsure", "Accept", "Reject", "Unsure", "ForwardLookingIdea", "Compilers", "Microarchitecture", "Systems"};
	...
	type Programme : Document, Contribution_File {
	...
	type Review : Document, Article, Thing, Conference_document {
	...
	type Workshop : Scientific_Event, Conference_part, Working_event, AcademicEvent, Event, Activity;

	type Tutorial : Individual_Presentation, Conference_part, Working_event, Activity, Lecture;
	...
	type Program_Committee : Committees;
	...
	type Paper_File : Domain_Topic, Contribution_File {
		Is_Type_Ref : Format?;
	};
	...
	type Speaker : Delegate, Person {
		Presentation_Ref : Document?;
	};
	...
	type Deadline_Abstract_Submission : Deadline;

	type Best_Applications_Paper_Award : Award;

	type Award {
		Awarded_by_Ref : Organizator*;
	};

	type Deadline_Paper_Submission : Deadline;

	type Deadline_Author_notification : Deadline;

	type Webmaster : Organizator;

	type Organizator : Person;

	type Author_of_paper_student : Author;

	type Best_Student_Paper_Award : Award;

	type ACM_SIGKDD : Thing {
		Hold_Ref : Conference?;
		Search_Ref : Sponzor?;
		Design_Ref : Deadline?;
	};
	...
	type Sponzor {
		Searched_by_Ref : ACM_SIGKDD?;
		Name_of_sponsor_Att : Text;
	};

	type Exhibitor : Sponzor;

	type Registration_SIGMOD_Member : Registration_fee;
	...
	type Bronze_Supporter : Sponzor;

	type Platinum_Supporter : Sponzor;

	type Conference_hall : Place;

	type Best_Paper_Awards_Committee : Committee;

	type Program_Chair : Organizator;

	type Main_office : Place;

	type @[Registration_Non-Member] : Registration_fee;

	type Sponzor_fee : Fee;

	type Registration_SIGKDD_Member : Registration_fee;

	type Program_Committee : Committee;

	type General_Chair : Organizator;

	type Gold_Supporter : Sponzor;

	type Registration_Student : Registration_fee;

	type Organizing_Committee : Committee;

	type Listener : Person;

	type Program_Committee_member : Organizator, Human;

	type Invited_Speaker : Speaker;

	type Best_Research_Paper_Award : Award;

	type Best_Student_Paper_Supporter : Sponzor;

	type Silver_Supporter : Sponzor;

	type Organizing_Committee_member : Organizator;

	type Author_of_paper : Author;
}