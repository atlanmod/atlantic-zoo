module PNML {
	...
	type Module : IdedElement {
		Interfaces_ContRef : Interface*;
		Netcontents_ContRef : NetContent*;
		Document_Ref : PNMLDocument;
	};

	type Interface {
		Module_Ref : Module;
		Importplace_Ref : Node*;
		Exportplace_Ref : Reference*;
		Importtransition_Ref : Node*;
		Exporttransition_Ref : Reference*;
	};
	...
	type Page : NetContent, IdedElement {
		Contents_ContRef : NetContent*;
		Tools_ContRef : ToolSpecific*;
		Pagegraphics_ContRef : PageGraphics?;
	};

	type Reference : Node {
		Instance_ContRef : Node?;
		Ref_ContRef : NCName;
		Importnode_Ref : ImportNode?;
	};

	type ReferencePlace : NetContent, Reference;

	type ReferenceTransition : NetContent, Reference;

	type Instance : NetContent, IdedElement {
		Ref_ContRef : URI;
		Importplace_ContRef : ImportNode*;
		Importtransition_ContRef : ImportNode*;
	};

	type ImportNode {
		Parameter_ContRef : NCName;
		Ref_ContRef : Reference;
		Instanceplace_Ref : Instance?;
		Instancetransition_Ref : Instance?;
	};
